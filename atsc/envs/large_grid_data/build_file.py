...
def build_large_grid(output_dir: str):
    # nod.xml file
    node = '  <node id="%s" x="%.2f" y="%.2f" type="%s"/>\n'
    write_file(os.path.join(output_dir, 'exp.nod.xml'), output_nodes(node))

    # typ.xml file
    write_file(os.path.join(output_dir, 'exp.typ.xml'), output_road_types())

    # edg.xml file
    edge = '  <edge id="%s" from="%s" to="%s" type="%s"/>\n'
    write_file(os.path.join(output_dir, 'exp.edg.xml'), output_edges(edge))

    # con.xml file
    con = '  <connection from="%s" to="%s" fromLane="%d" toLane="%d"/>\n'
    write_file(os.path.join(output_dir, 'exp.con.xml'), output_connections(con))

    # tls.xml file
    tls = '  <tlLogic id="%s" programID="0" offset="0" type="static">\n'
    phase = '    <phase duration="%d" state="%s"/>\n'
    write_file(os.path.join(output_dir, 'exp.tll.xml'), output_tls(tls, phase))

    # net config file
    write_file(os.path.join(output_dir, 'exp.netccfg'), output_netconfig())

    # generate net.xml file
    os.system('netconvert -c ' + os.path.join(output_dir, 'exp.netccfg'))

    # raw.rou.xml file
    write_file(os.path.join(output_dir, 'exp.rou.xml'), output_flows(1000, 2000, 0.2))
    ...
    write_file(os.path.join(output_dir, 'exp.add.xml'), output_ild(ild))

    # config file
    write_file(os.path.join(output_dir, 'exp.sumocfg'), output_config())

if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('output_dir')
    args = parser.parse_args()
    build_large_grid(args.output_dir)
